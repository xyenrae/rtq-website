...
const formSchema = z.object({
  ...
  tanggal_lahir: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Format tanggal salah (YYYY-MM-DD)"),
...
export default function RegistrationPage() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      alert(`Gagal mendaftar: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
    ...
    <div className="max-w-md mx-auto p-4">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          />
          {errors.no_telepon && (
            <p className="text-red-500 text-sm mt-1">{errors.no_telepon.message}</p>
          )}
        </div>
